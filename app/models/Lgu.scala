...
trait LguGen extends EntityCompanion[Lgu] {
  val simple = {
    ...
    get[Option[String]]("lgu_psgc") map {
      case id~level~parentLguId~parentRegionId~municipalityClass~psgc =>
        Lgu(id, level, parentLguId, parentRegionId, municipalityClass, psgc)
    }
  }
